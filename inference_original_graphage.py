"""
inference_original_graphage.py
-------------------------------
Load the original GraphAge checkpoint (no sequence features)
and generate:
...
Run on Spartan:
  ...
  python inference_original_graphage.py
"""
...
from torch.nn import Embedding, Linear, ModuleList, ReLU, Sequential, Dropout, SELU
...
CHECKPOINT_PATH = os.path.join(BASE_DIR, 'checkpoint/best_model_fold_2_thre_0.7 cur best 3.pth')
...
# Reproducibility
# ============================================================
SEED = 42   # original author uses 42
...
# ============================================================
# Hyperparameters (original author settings)
# ============================================================
THRESHOLD_CORR      = 0.70
...
# Original author node features: "1,2,3,4,5,6,8"
NODE_FEATURE_DICT = {
...
# ============================================================
# Graph construction (identical to original author)
# ============================================================
...
# ============================================================
# Feature processing (original author)
# ============================================================
...
# ============================================================
# DataLoader (original author -- no x_seq)
# ============================================================
...
# ============================================================
# Original author Net class (identical to graphage_final_version)
# ============================================================
class Net(torch.nn.Module):
    def __init__(self, deg):
        ...
        # Original author: for i in range(0) -- zero intermediate layers
        for i in range(0):
            conv = PNAConv(in_channels=INPUT_DIM, out_channels=INPUT_DIM,
                           aggregators=aggregators, scalers=scalers, deg=deg,
                           edge_dim=EDGE_DIM, towers=1, pre_layers=1, post_layers=1,
                           divide_input=False)
            self.convs.append(conv)
            if i % 3 == 0:
                self.batch_norms.append(BatchNorm(INPUT_DIM))

        self.LastLayer = PNAConv(in_channels=INPUT_DIM, out_channels=1,
                                  aggregators=aggregators, scalers=scalers, deg=deg,
                                  edge_dim=EDGE_DIM, towers=1, pre_layers=1, post_layers=1,
                                  divide_input=False)

        self.mlp = nn.Sequential(
            nn.Linear(number_of_cpgs, 1024), nn.ReLU(),
...
# ============================================================
# Data loading (original author split: random_state=42)
# ============================================================
...
test_loader = data_loader_maker(
    X_fold_test, y_fold_test,
    edge_index, edge_attr,
    filtered_information, batch_size=1
)
...
# Original author saves raw state_dict
if isinstance(ckpt, dict) and 'model_state_dict' in ckpt:
    state_dict = ckpt['model_state_dict']
    print(f'  Full checkpoint dict detected')
else:
    state_dict = ckpt
    print(f'  Raw state_dict detected')

model = Net(deg=deg).to(device)
...
pred_df  = pd.DataFrame({'true_age': truths, 'predicted_age': preds,
                          'error': preds - truths})
pred_csv = os.path.join(OUTPUT_DIR, 'test_predictions_original.csv')
...
ax.set_title('GraphAge (no sequence): Predicted vs True Age\n(Blood methylation, Test set)',
             fontsize=12)
...
scatter_path = os.path.join(OUTPUT_DIR, 'predicted_vs_true_age_original.png')
...
print(f'  - test_predictions_original.csv')
print(f'  - predicted_vs_true_age_original.png')
